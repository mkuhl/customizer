...
class CommentParser:
    ...
    def _extract_comment_content(self, line: str, comment_type: Optional[str] = None) -> Optional[str]:
    ...
    def _extract_template_marker(self, comment_content: str) -> Optional[tuple]:
        ...
        if match:
            ...
            if self._is_valid_variable_name(var_name) and self._is_valid_expression(expr):
    ...
    def _is_valid_variable_name(self, var_name: str) -> bool:
        ...
        # Must be valid Python identifier, not start with underscore, and not be a keyword
        return (var_name.isidentifier() and 
                not var_name.startswith('_') and 
                not keyword.iskeyword(var_name))
    ...
    def get_marker_statistics(self, markers: List[TemplateMarker]) -> Dict[str, int]:
        ...
        stats = {
            ...
            'unique_variables': len(set(m.variable_name for m in markers)),
            'comment_types': {}
        }
    ...
    def validate_template_syntax(self, template_expression: str) -> bool:
        ...
        try:
            ...
            template = env.from_string(f"{{{{ {template_expression} }}}}")
            return True
        except (TemplateSyntaxError, Exception):
            return False
    
    def validate_template_syntax_detailed(self, template_expression: str) -> tuple[bool, Optional[str]]:
    ...
    def validate_all_markers(self, markers: List[TemplateMarker]) -> Dict[str, List[str]]:
        ...
        for marker in markers:
            is_valid, error = self.validate_template_syntax_detailed(marker.template_expression)
