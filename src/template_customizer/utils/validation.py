            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', key):
                        errors.append(f"Invalid parameter name '{current_path}': must be valid identifier")
...
class TemplateValidator:
    ...
    def validate_variable_reference(
        ...
        variable_pattern = re.compile(r'values\.([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)')
...
class ProjectValidator:
    ...
    def validate_config_file(self, config_path: Path) -> List[str]:
        ...
        if config_path.suffix.lower() not in valid_extensions:
            errors.append(
                f"Configuration file should have one of these extensions: {', '.join(valid_extensions)}"
            )
