...
class FileWriter:
    ...
    def prepare_changes(
        self, file_path: Path, markers_and_values: List[tuple]
    ) -> List[FileChange]:
        ...
        try:
            ...
            raise ValueError(f"Cannot read file as text: {file_path}") from e
        ...
        for marker, rendered_value in markers_and_values:
            ...
            if target_line_idx < len(lines):
                ...
                new_content = self._generate_new_line(
                    old_content, marker.variable_name, rendered_value
                )
    ...
    def create_backup(self, file_path: Path) -> Path:
        ...
        if self.backup_dir is None:
            self.backup_dir = Path(
                tempfile.mkdtemp(prefix="template_customizer_backup_")
            )
    ...
    def _generate_new_line(
        self, old_line: str, variable_name: str, new_value: str
    ) -> str:
